...
# reference strings
class TestSoftCallback < RubTap::TestCase
	def global_setup
	...
	def test_bo_record
		do_test(@client_reference, create_reference("bo"), Plumber::GenericCommand.new(@bo_client))
	end

	def test_ao_record
		do_test(@client_reference, create_reference("ao"), Plumber::GenericCommand.new(@ao_client))
	end

	def test_calcout_record
		do_test(@client_reference, create_reference("calcout"), Plumber::GenericCommand.new(@calcout_client))
	end

	def test_longout_record
		do_test(@client_reference, create_reference("longout"), Plumber::GenericCommand.new(@longout_client))
	end

	def test_mbbo_record
		do_test(@client_reference, create_reference("mbbo"), Plumber::GenericCommand.new(@mbbo_client))
	end

	def test_mbboDirect_record
		do_test(@client_reference, create_reference("mbboDirect"), Plumber::GenericCommand.new(@mbboDirect_client))
	end

	def test_stringout_record
		do_test(@client_reference, create_reference("stringout"), Plumber::GenericCommand.new(@stringout_client))
	end
